...
| 3 | **New.** `Delivery#complete()` has no status guard, but `DeliveryCommandServiceImpl#handle(CompleteDeliveryCommand)` re-runs `FuelOrder#receive()`, which *does* guard on `OrderStatus.DISPATCHED`. After the first `/complete` the order is `PENDING_PAYMENT`, so retrying `/complete` on the same delivery throws a raw `IllegalStateException` → `500` instead of `409`. Because the handler is `@Transactional`, the retry's redundant equipment-refuel and driver/vehicle-status writes roll back with the exception — no double side effects survive, only the wrong status code. | `StateLifecycleRetryCharacterizationTest#completingAnAlreadyDeliveredDeliveryReturns500InsteadOf409` | MEDIUM | S14 (Delivery lifecycle with `ASSIGNED→STARTED→ARRIVED→DELIVERING→COMPLETED`, retry-safe) |
